...
public class BookingJDBCRepository {
    ...
    public List<Trip> findAllTripsAfter(LocalDateTime now) {
        try {
            String sql = "SELECT TripId, TripName, Duration, DepartureTime, Destinations, Availability, Description, BasePrice " +
                    "FROM Trip WHERE DepartureTime > ? AND Availability > 0 ORDER BY DepartureTime";

            System.out.println("=== DEBUG: Repository.findAllTripsAfter() ===");
            System.out.println("SQL: " + sql);
            System.out.println("Now parameter: " + now);

            List<Trip> trips = jdbcTemplate.query(sql, new TripRowMapper(), now);
            System.out.println("Trips found in DB: " + trips.size());

            // Debug: Print each trip found
            for (Trip trip : trips) {
            ...
            System.out.println("ERROR in repository: " + e.getMessage());
            e.printStackTrace();
            return Collections.emptyList();
        }
    }

    // ... REST OF YOUR EXISTING METHODS (Customer methods, Booking methods, Payment methods, RowMappers) ...
    // Customer methods
    ...
    private static class TripRowMapper implements RowMapper<Trip> {
        ...
        public Trip mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            System.out.println("DEBUG - Mapped Trip: " + trip.getTripName() +
                    " | Departure: " + trip.getDepartureTime() +
                    " | Available: " + trip.getAvailability());
